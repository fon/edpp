...
public class TestPastryNode {
	...
	public TestPastryNode(int bindport, InetSocketAddress bootaddress,
			...
			if (c == null) {
				...
				while (true) {
					...
					if (items.get(0).equals("sample")) {
						if (items.size() != 3) {
							System.out
									.println("Must give two arguments - (sample numOfExecutions numOfRounds)");
							continue;
						}
						int numberOfExecutions = Integer.parseInt(items.get(1));
						int numberOfRounds = Integer.parseInt(items.get(2));
						Session s = dt.requestSamplingData(numberOfExecutions,
								numberOfRounds);
						System.out.println("I know about session "
								+ s.getSessionId());
						...
						System.out
								.println("Accepted commands are sample and exit...");
					}
				}
			}
		} catch (Exception e) {
